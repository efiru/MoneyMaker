...
public:
    ...
    void upgrade(const int nr_upgrades) {
        ...
        if (it != ordineBancnote.end()) {
            ...
            index = std::min(index + nr_upgrades , (int)(ordineBancnote.size() - 1));
...
public:
    ...
    void aruncaBancnota() {
        doubleTapUnlocked ? (clicksCurent+=2 ,  clicksTotal+=2) : (clicksCurent++ , clicksTotal++);
    ...
    [[nodiscard]] int getClicks() const { return clicksCurent; }
    ...
    void checkUnlock(const Player& p) {
        ...
        switch (tip) {
            ...
            case AchievementType::DOUBLE_TAP:
                if (p.getDoubleTap() >= prag) activat = true;
                break;
        }
    };
